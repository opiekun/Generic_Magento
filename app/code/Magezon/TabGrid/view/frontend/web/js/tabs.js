...
(function (factory) {
    ...
    // magezon.tabs base functionality
    $.widget('magezon.tabs', $.ui.tabs, {
        options: {
            spinner: false,
            groups: null,
            tabPanelClass: '',
    ...
    // Extension for magezon.tabs - Move panels in destination element
    $.widget('magezon.tabs', $.magezon.tabs, {
        /**
    ...
    // Extension for magezon.tabs - Ajax functionality for tabs
    $.widget('magezon.tabs', $.magezon.tabs, {
        options: {
            ajaxOptions: {
                data: {
            ...
            load: function(event, ui) {
                ...
                $(ui.tab).find('a').eq(0).prop('href', '#' + panel.prop('id'));
                panel.trigger('contentUpdated');
            }
        }
    });

    // Extension for magezon.tabs - Attach event handlers to tabs
    $.widget('magezon.tabs', $.magezon.tabs, {
        options: {
            tabIdArgument: 'tab',
        ...
        _onInvalid: function(e) {
            ...
            var targetElement = fakeEvent.currentTarget.attr('href');
            var errorElement = $(targetElement).find('.required-entry[aria-invalid=true]:not(:disabled)').eq(0);

            var parent = errorElement.parents('.item-content').eq(0);
            $(targetElement).find('div').removeClass('_current');
            if (!parent.hasClass('_active') && !parent.hasClass('active')) {
                var uiId = parent.attr("data-ui-id");
                if (uiId) {
                    $("#tab_" + uiId).trigger('click');
                }
            }
            ...
            errorElement.addClass('_current');
            errorElement.focus();
        },
    ...
    // Extension for magezon.tabs - Shadow tabs functionality
    $.widget('magezon.tabs', $.magezon.tabs, {
        /**
