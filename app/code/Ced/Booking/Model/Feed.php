...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH = 'system/booking/feed_url';
    const XML_FREQUENCY_PATH = 'system/booking/frequency';
    const XML_LAST_UPDATE_PATH = 'system/booking/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_feedHelper->getEnvironmentInformation());

        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));

        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules = $this->_feedHelper->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {

                if (!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if ($this->_feedHelper->isAllowedFeedType($item)) {
